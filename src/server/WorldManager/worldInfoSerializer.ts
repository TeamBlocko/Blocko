	...
	serializeInfo(info: WorldInfo | WorldSettings): SerializedWorldInfo {
		...
		for (const [property, value] of pairs(info)) {
			if (typeIs(value, "table"))
				result[property] = this.serializeInfo(value) as never
			else
				result[property] = [this.serialize(value), typeOf(value)] as never
		}
	...
	deserializeInfo(info: object): WorldInfo {
		...
		for (const [property, value] of pairs(info)) {
			if (t.array(t.string)(value))
				result[property] = this.deserialize(value[0], value[1]) as never
			else
				result[property] = this.deserializeInfo(value) as never
		}
