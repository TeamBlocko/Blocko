...
function WorldInfo(props: WorldInfoPropTypes) { 
	return (
		<frame BackgroundTransparency={1} Size={new UDim2(0.95, 0, 0, 380)}>
			...
			<InputBox Name="Name" Length={60} Default={props.Name} OnChange={(newValue) => props.OnInputBoxUpdate("Name", newValue)}
				HandleInput={(input) => input.size() > 62 ? input.sub(0, 62) : input}/>
			<InputBox Name="Description" Length={135} Default={props.Description} OnChange={(newValue) => props.OnInputBoxUpdate("Description", newValue)}
